...
import { readProFile } from '../utils/proParser';
import { readChordProFile } from '../utils/chordproIO';
...
export default function SongEditor({ song, onSave, onDelete, onCancel }: SongEditorProps) {
  ...
  const handleImportFile = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      if (['cho', 'chordpro', 'chopro', 'crd'].includes(ext)) {
        const result = await readChordProFile(file);
        if (result.title) setTitle(result.title);
        if (result.artist) setArtist(result.artist);
        if (result.key) setOriginalKey(result.key);
        if (result.bpm) setBpmInput(String(result.bpm));
        ...
        // ProPresenter formats
        const result = await readProFile(file);
        if (result.title) setTitle(result.title);
        if (result.artist) setArtist(result.artist);
        if (result.content) {
          setContent(result.content);
      ...
      logger.error('Failed to parse imported file:', err);
      setImportError(t('worship.importError'));
    }
        ...
        {!song && (
          <div className="mb-2">
            <input
              ref={fileInputRef}
              type="file"
              accept=".cho,.chordpro,.chopro,.crd,.pro,.pro4,.pro5,.pro6,.pro7"
              onChange={handleImportFile}
              className="hidden"
              id="song-file-input"
              aria-label={t('worship.importFile')}
            />
            <label
              htmlFor="song-file-input"
              className="inline-flex items-center gap-2 px-4 py-2 text-sm font-medium rounded-lg cursor-pointer bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-gray-600 transition"
            >
              <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                <path strokeLinecap="round" strokeLinejoin="round" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-8l-4-4m0 0L8 8m4-4v12" />
              </svg>
              {t('worship.importFile')}
            </label>
            <p className="text-xs text-gray-400 dark:text-gray-500 mt-1">{t('worship.importFileHint')}</p>
            {importError && (
