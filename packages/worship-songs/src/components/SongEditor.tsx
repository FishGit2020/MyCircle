...
export default function SongEditor({ song, onSave, onDelete, onCancel }: SongEditorProps) {
  ...
  const handleImportPro = async (e: React.ChangeEvent<HTMLInputElement>) => {
    setImportError('');
        ...
        {!song && (
          <div className="mb-2">
            <input
              ref={fileInputRef}
              type="file"
              accept=".pro,.pro4,.pro5,.pro6,.pro7"
              onChange={handleImportPro}
              className="hidden"
              id="pro-file-input"
              aria-label={t('worship.importPro')}
            />
            <label
              htmlFor="pro-file-input"
            ...
            <p className="text-xs text-gray-400 dark:text-gray-500 mt-1">{t('worship.importProHint')}</p>
            {importError && (
