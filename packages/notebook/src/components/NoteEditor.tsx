...
export default function NoteEditor({ note, onSave, onCancel, onDelete, onPublish }: NoteEditorProps) {
  ...
  const handleContentChange = useCallback((e: React.ChangeEvent<HTMLTextAreaElement>) => {
    ...
    // Auto-save on content change if enabled and has content
    if (autosave && (title.trim() || hasText)) {
      if (autosaveTimeoutRef.current) clearTimeout(autosaveTimeoutRef.current);
      autosaveTimeoutRef.current = setTimeout(async () => {
        if (title.trim() || hasText) {
          try {
            await onSave(note?.id ?? null, { title: title.trim(), content: contentRef.current.trim() });
          } catch { /* ignore autosave errors */ }
        }
      }, 2000);
    }
  }, [autosave, title, hasContent, note, onSave]);

  const handleTitleChange = useCallback((newTitle: string) => {
    setTitle(newTitle);

    // Auto-save on title change if enabled
    if (autosave && (newTitle.trim() || hasContent)) {
      if (autosaveTimeoutRef.current) clearTimeout(autosaveTimeoutRef.current);
      autosaveTimeoutRef.current = setTimeout(async () => {
        if (newTitle.trim() || hasContent) {
          try {
            await onSave(note?.id ?? null, { title: newTitle.trim(), content: contentRef.current.trim() });
          } catch { /* ignore autosave errors */ }
        }
      }, 2000);
    }
  }, [autosave, hasContent, note, onSave]);
      ...
      <div className="flex items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2 pt-6">
          <button
            ...
            title={autosave ? t('notebook.autosaveOn' as any) : t('notebook.autosaveOff' as any)} // eslint-disable-line @typescript-eslint/no-explicit-any
          >
            {autosave ? '💾' : '✋'}
          </button>
        </div>
