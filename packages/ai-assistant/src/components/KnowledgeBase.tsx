...
import { useTranslation, useMutation, useQuery, useLazyQuery, INGEST_KNOWLEDGE_DOC, GET_KNOWLEDGE_SOURCES, RAG_SEARCH, GET_BENCHMARK_ENDPOINTS, GET_BENCHMARK_ENDPOINT_MODELS, GET_OLLAMA_MODELS } from '@mycircle/shared';
...
export default function KnowledgeBase({ onAskAi }: KnowledgeBaseProps) {
  ...
  const handleIngest = useCallback(async () => {
    ...
    if (!title.trim()) {
      setIngestMessage({ type: 'error', text: t('ai.knowledgeBase.titleRequired') });
      return;
    }
    if (!content.trim() || content.trim().length < 50) {
      setIngestMessage({ type: 'error', text: t('ai.knowledgeBase.contentTooShort') });
      return;
    }
    try {
      ...
      const count = data?.ingestKnowledgeDoc?.chunkCount ?? 0;
      setIngestMessage({ type: 'success', text: t('ai.knowledgeBase.ingestSuccess', { count }) });
      setTitle('');
      setContent('');
      setSourceUrl('');
    } catch {
  ...
  const [executeSearch, { loading: searching }] = useLazyQuery(RAG_SEARCH, {
    ...
    onCompleted: (data) => {
      setSearchResults(data?.ragSearch ?? []);
    },
  });
  ...
  const handleAskAi = useCallback(() => {
    ...
    const contextStr = `Use the following knowledge base excerpts to answer the user's question:\n\n${contextParts.join('\n\n---\n\n')}`;
    onAskAi(contextStr);
    ...
    <div className="flex-1 flex flex-col min-h-0 overflow-y-auto space-y-6">
      ...
      <section aria-labelledby="kb-ingest-heading">
        ...
        <div className="space-y-2">
          <div>
            <label htmlFor="kb-title" className="block text-xs font-medium text-gray-600 dark:text-gray-400 mb-1">
              {t('ai.knowledgeBase.titleLabel')}
            </label>
            <input
              id="kb-title"
              type="text"
              ...
              placeholder={t('ai.knowledgeBase.titlePlaceholder')}
              maxLength={200}
          ...
          <div>
            <label htmlFor="kb-content" className="block text-xs font-medium text-gray-600 dark:text-gray-400 mb-1">
              {t('ai.knowledgeBase.contentLabel')}
            </label>
            <textarea
              id="kb-content"
              value={content}
              onChange={e => setContent(e.target.value)}
              placeholder={t('ai.knowledgeBase.contentPlaceholder')}
              rows={6}
          ...
          <div>
            <label htmlFor="kb-source-url" className="block text-xs font-medium text-gray-600 dark:text-gray-400 mb-1">
              {t('ai.knowledgeBase.sourceUrlLabel')}
            </label>
            <input
              id="kb-source-url"
              type="url"
              ...
              placeholder={t('ai.knowledgeBase.sourceUrlPlaceholder')}
              className="w-full px-3 py-2 text-sm rounded border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 focus:outline-none focus:ring-1 focus:ring-blue-400"
            />
          </div>
          <button
            ...
            disabled={ingesting || !title.trim() || !content.trim() || !embedModel}
      ...
      <section aria-labelledby="kb-search-heading">
        <h2 id="kb-search-heading" className="text-sm font-semibold text-gray-800 dark:text-gray-200 mb-2">
          {t('ai.knowledgeBase.searchTitle')}
        </h2>
        <div className="flex flex-col md:flex-row gap-2 mb-2">
          <input
            ...
            aria-label={t('ai.knowledgeBase.searchTitle')}
          />
          <div className="flex items-center gap-2">
            <label htmlFor="kb-topk" className="text-xs text-gray-500 dark:text-gray-400 whitespace-nowrap">
              {t('ai.knowledgeBase.topKLabel')}
            </label>
            <select
              id="kb-topk"
              value={topK}
            ...
            <button
              ...
              disabled={searching || !question.trim() || !embedModel}
              className="px-4 py-2 text-sm font-medium rounded bg-blue-600 text-white hover:bg-blue-700 dark:bg-blue-500 dark:hover:bg-blue-600 disabled:opacity-50 disabled:cursor-not-allowed transition-colors min-h-[44px]"
            >
        ...
        {hasModelMismatch && (
          <p className="text-xs text-amber-600 dark:text-amber-400 mb-2">
            {t('ai.knowledgeBase.modelMismatchWarning')}
          </p>
        )}
        ...
        {searchResults.length > 0 && (
          <div className="space-y-3">
                ...
                <div key={result.id} className="p-3 rounded border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800">
                  <div className="flex items-center justify-between mb-1">
                    <span className="text-xs font-medium text-gray-500 dark:text-gray-400">
                      ...
                      {result.sourceUrl && (
                        <> &middot; <a href={result.sourceUrl} target="_blank" rel="noopener noreferrer" className="text-blue-500 dark:text-blue-400 hover:underline">{result.sourceUrl}</a></>
                      )}
                    </span>
                    <span className="text-xs font-mono text-gray-500 dark:text-gray-400">
                      {t('ai.knowledgeBase.resultScore', { score: Math.round(result.score * 100) })}
                    </span>
                  </div>
                  <p className="text-sm text-gray-800 dark:text-gray-200 whitespace-pre-wrap">
                    {displayText}{!isExpanded && result.text.length > 200 && '...'}
                  </p>
            ...
            <button
              type="button"
              onClick={handleAskAi}
              disabled={!onAskAi || searchResults.length === 0}
      ...
      <section aria-labelledby="kb-sources-heading">
        <h2 id="kb-sources-heading" className="text-sm font-semibold text-gray-800 dark:text-gray-200 mb-2">
          {t('ai.knowledgeBase.sourcesTitle')}
        </h2>
        {sources.length === 0 ? (
            ...
            {sources.map((source: { id: string; title: string; sourceUrl: string | null; chunkCount: number; embedModel: string; createdAt: string }) => (
              <div key={source.id} className="flex flex-col md:flex-row md:items-center md:justify-between p-3 rounded border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800 gap-1">
                <div className="flex-1 min-w-0">
                  ...
                  {source.sourceUrl && (
                    <a href={source.sourceUrl} target="_blank" rel="noopener noreferrer" className="text-xs text-blue-500 dark:text-blue-400 hover:underline truncate block">
                      {source.sourceUrl}
                    </a>
                  )}
                </div>
