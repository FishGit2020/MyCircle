...
export interface UserProfile {
  ...
  podcastProgress?: Record<string, { position: number; duration: number }>;
...
export async function updatePodcastProgress(uid: string, progress: Record<string, { position: number; duration: number }> | null) {
  if (!db) return;
  const userRef = doc(db, 'users', uid);
  await updateDoc(userRef, {
    podcastProgress: progress || null,
    updatedAt: serverTimestamp(),
  });
}
...
// Daily Log — user-scoped subcollection
export async function getDailyLogEntries(uid: string) {
  if (!db) return [];
...
export async function deleteDailyLogEntry(uid: string, entryId: string) {
  if (!db) throw new Error('Firebase not initialized');
  ...
  window.__resumeTailor = {
    uploadAndParse: async (fileName: string, fileBase64: string, contentType: string, model: string, endpointId?: string | null) => {
      ...
      const res = await fetch('/resume-tailor/upload', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token}` },
        body: JSON.stringify({ fileName, fileBase64, contentType: mimeType, model, endpointId: endpointId ?? null }),
      });
      if (!res.ok) {
        const err = await res.json().catch(() => ({ error: 'Upload failed' }));
        throw new Error(err.error || 'Upload failed');
