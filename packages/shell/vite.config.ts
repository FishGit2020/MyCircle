...
function firebaseMessagingSW(): PluginOption {
  ...
  function generate(): string {
    return [
      ...
      '// Only show a manual notification for data-only messages.',
      '// Messages with a "notification" field are auto-displayed by FCM.',
      'messaging.onBackgroundMessage((payload) => {',
      '  if (payload.notification) return;',
      '  const title = payload.data?.title || "MyCircle";',
...
const immigrationTrackerRemote = isProduction
...
const familyGamesRemote = isProduction
...
const radioStationRemote = isProduction
